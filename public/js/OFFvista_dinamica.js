document.addEventListener('DOMContentLoaded', () => {
  console.log('🧪 JS caricato correttamente: dentro DOMContentLoaded');

  const filtro = new URLSearchParams(window.location.search).get('filtro');

  // Nascondi filtri se vista speciale
  const vistaSpeciale = ['pagati', 'consegnati', 'da_fatturare'].includes(filtro);
  if (vistaSpeciale) {
    const bloccoFiltri = document.getElementById('filtri');
    if (bloccoFiltri) bloccoFiltri.style.display = 'none';
  }

  // Titolo dinamico
  const titolo = document.getElementById('titolo-vista');
  if (titolo) {
    let vistaLabel = '';
    switch (filtro) {
      case 'pagati':
        vistaLabel = 'Vista: Pagamenti Completati';
        break;
      case 'consegnati':
        vistaLabel = 'Vista: Lavori Consegnati';
        break;
      case 'da_fatturare':
        vistaLabel = 'Vista: Da Fatturare';
        break;
      default:
        vistaLabel = 'Vista Generale';
    }
    titolo.innerHTML = `Flusso Produzione <span class="vista-verde">${vistaLabel}</span>`;
  }
  ...
  if (successParam === 'true') {
    ...
    alertBox.textContent = '✅ Modifiche salvate con successo!';
  ...
  console.log('📍 fetch attivato');
  console.log('🔍 Richiesta fetch verso:', endpoint);

  fetch(endpoint)
    .then(res => {
      console.log('📥 Risposta fetch ricevuta:', res);
      return res.json();
    })
    .then(dati => {
      console.log('📦 Dati ricevuti correttamente:', dati);
      window.produzioneDati = dati;

      if (typeof aggiornaTabella === 'function') {
        console.log('✅ aggiornaTabella disponibile, procedo');
        aggiornaTabella(dati);
      } else {
        console.warn('⚠️ Funzione aggiornaTabella non trovata');
      }
    })
    .catch(error => {
      console.error('❌ Errore nel caricamento dei dati da', endpoint, ':', error);
    });
