...
function initVistaDinamica() {
  ...
  //inizio inserimento codice AI per i filtri
// === RIFERIMENTI FILTRI ===
const $ = (id) => document.getElementById(id);
const filtroNome = $('filtro-nome');
const filtroTipoCliente = $('filtro-tipo-cliente');
const filtroUrgente = $('filtro-urgente');
const filtroTipoLavorazione = $('filtro-tipo-lavorazione');
const filtroTipoLinea = $('filtro-tipo-linea');
const filtroRAL1 = $('filtro-ral1');
const filtroRAL2 = $('filtro-ral2');
const filtroVernice = $('filtro-vernice');
const filtroIngresso = $('filtro-ingresso');
const filtroSmontaggio = $('filtro-smontaggio');
const filtroFalegnameria = $('filtro-falegnameria');
const filtroFinitoAdmin = $('filtro-finito-admin');
const filtroConsegna = $('filtro-consegna');
const filtroProgressivo = $('filtro-progressivo');
const filtroACorpo = $('filtro-a-corpo');
const filtroTrasparente = $('filtro-trasparente');
const filtroStandby = $('filtro-standby');
const filtroNomeVuoto = $('filtro-nome-vuoto');
const resetFiltri = $('reset-filtri');

// === PRECOMPILA DA LOCALSTORAGE ===
if (filtroNome) filtroNome.value = localStorage.getItem('filtroNome') || '';
if (filtroTipoCliente) filtroTipoCliente.value = localStorage.getItem('filtroTipoCliente') || '';
if (filtroUrgente) filtroUrgente.checked = localStorage.getItem('filtroUrgente') === 'true';
if (filtroTipoLavorazione) filtroTipoLavorazione.value = localStorage.getItem('filtroTipoLavorazione') || '';
if (filtroTipoLinea) filtroTipoLinea.value = localStorage.getItem('filtroTipoLinea') || '';
if (filtroRAL1) filtroRAL1.value = localStorage.getItem('filtroRAL1') || '';
if (filtroRAL2) filtroRAL2.value = localStorage.getItem('filtroRAL2') || '';
if (filtroVernice) filtroVernice.checked = localStorage.getItem('filtroVernice') === 'true';
if (filtroIngresso) filtroIngresso.value = localStorage.getItem('filtroIngresso') || '';
if (filtroSmontaggio) filtroSmontaggio.value = localStorage.getItem('filtroSmontaggio') || '';
if (filtroFalegnameria) filtroFalegnameria.value = localStorage.getItem('filtroFalegnameria') || '';
if (filtroFinitoAdmin) filtroFinitoAdmin.value = localStorage.getItem('filtroFinitoAdmin') || '';
if (filtroConsegna) filtroConsegna.value = localStorage.getItem('filtroConsegna') || '';
if (filtroProgressivo) filtroProgressivo.value = localStorage.getItem('filtroProgressivo') || '';
if (filtroACorpo) filtroACorpo.checked = localStorage.getItem('filtroACorpo') === 'true';
if (filtroTrasparente) filtroTrasparente.checked = localStorage.getItem('filtroTrasparente') === 'true';
if (filtroStandby) filtroStandby.checked = localStorage.getItem('filtroStandby') === 'true';
if (filtroNomeVuoto) {
  const val = localStorage.getItem('filtroNomeVuoto');
  if (val) filtroNomeVuoto.value = val;
  else { filtroNomeVuoto.value = 'pieni'; localStorage.setItem('filtroNomeVuoto', 'pieni'); }
}

// === SALVA + REFRESH ===
function salvaFiltri() {
  localStorage.setItem('filtroNome', filtroNome?.value || '');
  localStorage.setItem('filtroTipoCliente', filtroTipoCliente?.value || '');
  localStorage.setItem('filtroUrgente', filtroUrgente?.checked || false);
  localStorage.setItem('filtroTipoLavorazione', filtroTipoLavorazione?.value || '');
  localStorage.setItem('filtroTipoLinea', filtroTipoLinea?.value || '');
  localStorage.setItem('filtroRAL1', filtroRAL1?.value || '');
  localStorage.setItem('filtroRAL2', filtroRAL2?.value || '');
  localStorage.setItem('filtroVernice', filtroVernice?.checked || false);
  localStorage.setItem('filtroIngresso', filtroIngresso?.value || '');
  localStorage.setItem('filtroSmontaggio', filtroSmontaggio?.value || '');
  localStorage.setItem('filtroFalegnameria', filtroFalegnameria?.value || '');
  localStorage.setItem('filtroFinitoAdmin', filtroFinitoAdmin?.value || '');
  localStorage.setItem('filtroConsegna', filtroConsegna?.value || '');
  localStorage.setItem('filtroProgressivo', filtroProgressivo?.value || '');
  localStorage.setItem('filtroACorpo', filtroACorpo?.checked || false);
  localStorage.setItem('filtroTrasparente', filtroTrasparente?.checked || false);
  localStorage.setItem('filtroStandby', filtroStandby?.checked || false);
  localStorage.setItem('filtroNomeVuoto', filtroNomeVuoto?.value || 'tutti');

  if (window.produzioneDati && window.aggiornaTabella) window.aggiornaTabella(window.produzioneDati);
}

// === LISTENER come in STABILE ===
const filtriInput = [
  filtroNome, filtroTipoCliente, filtroTipoLavorazione, filtroTipoLinea,
  filtroRAL1, filtroRAL2, filtroIngresso, filtroSmontaggio,
  filtroFalegnameria, filtroFinitoAdmin, filtroConsegna, filtroProgressivo, filtroNomeVuoto
];
filtriInput.forEach(el => el && el.addEventListener('input', salvaFiltri));

[filtroUrgente, filtroVernice, filtroACorpo, filtroTrasparente, filtroStandby]
  .forEach(el => el && el.addEventListener('change', salvaFiltri));

resetFiltri && resetFiltri.addEventListener('click', () => {
  localStorage.removeItem('filtroNome');
  localStorage.removeItem('filtroTipoCliente');
  localStorage.removeItem('filtroUrgente');
  localStorage.removeItem('filtroTipoLavorazione');
  localStorage.removeItem('filtroTipoLinea');
  localStorage.removeItem('filtroRAL1');
  localStorage.removeItem('filtroRAL2');
  localStorage.removeItem('filtroVernice');
  localStorage.removeItem('filtroIngresso');
  localStorage.removeItem('filtroSmontaggio');
  localStorage.removeItem('filtroFalegnameria');
  localStorage.removeItem('filtroFinitoAdmin');
  localStorage.removeItem('filtroConsegna');
  localStorage.removeItem('filtroProgressivo');
  localStorage.removeItem('filtroACorpo');
  localStorage.removeItem('filtroTrasparente');
  localStorage.removeItem('filtroStandby');
  localStorage.setItem('filtroNomeVuoto', 'pieni');
  if (window.produzioneDati && window.aggiornaTabella) window.aggiornaTabella(window.produzioneDati);
});

  //fine inserimento codice AI per i filtri

  // Nota: il fetch + render e il menu dinamico sono gestiti nelle sezioni
  // successive (fuori da init) che attendono anche ordinaDati.
}
...
// === Menu dinamico ===
if (false) fetch('partials/menu_base.html')
  .then(res => res.text())
  .then(base => {
    const menuContainer = document.getElementById('menu-container');
    if (menuContainer) menuContainer.innerHTML = base;
  ...
  .then(linkHtml => {
    ...
    // === TOPNAV: padding dinamico per evitare sovrapposizione contenuti ===
    function applyTopnavPadding() {
      const topnav = document.querySelector('.top-nav');
      const h = topnav ? topnav.offsetHeight : 64; // fallback
    ...
    // 2) Aggiorna su resize
    window.addEventListener('resize', applyTopnavPadding);

    // 3) Aggiorna se il topnav cambia altezza (submenu, wrap, ecc.)
    const topnavEl = document.querySelector('.top-nav');
    if (topnavEl && 'ResizeObserver' in window) {
      const ro = new ResizeObserver(applyTopnavPadding);
      ro.observe(topnavEl);
    }
  });
...
const totOre = sommaOre.toFixed(2);   // per la cella
item.totale_ore = sommaOre;           // per l’ordinamento
